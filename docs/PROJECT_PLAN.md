...
- The conversation _is_ the order, and when the conversation is lost, the order is lost.
...
- Multi-language / multi-currency _storefronts_ (the schema will be i18n- and multi-currency-ready from day one; only one active locale/currency ships in v1).
...
**Why:** Bootstrap's design language is instantly recognisable and generic; escaping it costs more than starting from primitives. Tailwind gives a constraint system (spacing, type, colour scales) that _enforces_ consistency, ships almost no unused CSS, and the v4 engine is already configured via `@tailwindcss/vite`.
...
**Why not Laravel Cashier:** Cashier is excellent for _subscriptions_ and will be used for **vendor subscription plans**. It is not designed for marketplace split payments, so order payments go through our own driver layer.
...
**Why:** microservices for a pre-launch marketplace buy distributed-systems problems and no delivery speed. The module boundaries preserve the _option_ to extract a service (search, chat, media) later, when there is a measured reason.
...
**Vendor** is an _organisation_ (a store), not a person. Users are attached to a vendor with a vendor-scoped role:
...
_As a guest, I want to browse and search the whole catalog without an account so that I can evaluate the marketplace before committing._
...
_As a guest, I want to buy without creating an account so that I can order a gift quickly._
...
_As a guest, I want to track my order with an order number and email so that I don't need an account._
...
_As a customer, I want to see the maker, their verification, ratings, lead time, and shipping cost before I add to cart so that I trust the purchase._
...
_As a customer, I want to add my own text/colour/date to a product so that the gift feels personal._
...
_As a customer, I want to describe a bespoke piece with reference images and get a quote so that I can commission work._
...
_As a customer, I want to buy from three makers in one checkout so that I pay once._
...
_As a customer, I want to message a vendor about an order so that I can ask for changes._
...
_As a customer, I want to request a return so that I can get a refund for a damaged item._
...
_As a customer, I want addresses, wishlists, order history, reviews, and notification settings in one place._
...
_As a customer, I want to review a delivered product with photos so that I can help other buyers._
...
_As a maker, I want a guided setup so that I can start selling the same day._
...
_As a vendor, I want to publish a product with photos, variants, and lead time in a few minutes._
...
**US-V3 — Manage orders**
_As a vendor, I want a single queue of orders that need action so that nothing is missed._
...
_As a vendor, I want to send a structured quotation with revisions so that expectations are recorded._
...
_As a vendor, I want analytics on sales, traffic, conversion, and best sellers so that I can decide what to make next._
**AC:** date-range comparison vs previous period; revenue net of commission; export CSV; loads under 1s from cache.

**US-V6 — Get paid**
_As a vendor, I want a clear earnings breakdown and a way to withdraw so that I trust the platform with my money._
...
**US-V7 — Stock control**
_As a vendor, I want stock to decrement on sale and warn me when low so that I don't oversell one-of-a-kind pieces._
...
**US-V8 — Delegate**
_As a vendor owner, I want to add staff with limited access so that my assistant can pack orders without seeing payouts._
...
- Every public class and method has a docblock explaining _why_, not restating the signature.
...
| Customer | After `in_production`    | Cancellation _request_; vendor accepts (refund per policy) or declines with a reason |
...
3. **Policies** — the final authority on _this_ record: ownership, vendor scope, state (you cannot refund a cancelled order), and amount thresholds.
...
- Frontend permission props are for _UI convenience only_; the server re-checks everything. A hidden button is not a security control.
...
- Octane is a _later_ option (Milestone 14), not a v1 dependency — correctness of shared state first.
...
### M0 — Foundation & tooling _(~3 days)_
...
### M1 — Design system & app shell _(~5 days)_
...
### M2 — Identity & accounts _(~5 days)_
...
### M3 — Vendor onboarding _(~6 days)_
...
### M4 — Catalog _(~10 days — the biggest build)_
...
### M5 — Cart & the multi-vendor order model _(~7 days — the critical one)_
...
### M7 — Payments, commission & ledger _(~8 days)_
...
### M8 — Fulfilment & customer orders _(~6 days)_
...
### M11 — Reviews, wishlists & discovery _(~5 days)_
...
### M13 — Notifications & preferences _(~4 days)_
...
### M14 — Vendor analytics, earnings & payouts _(~6 days)_
...
### M15 — Admin & platform operations _(~7 days)_
...
### M16 — Promotions & growth _(~5 days)_
...
### M17 — Hardening & launch _(~8 days)_
...
_End of PROJECT_PLAN.md — Step 1 complete. Awaiting approval before creating `TASKS.md` (Step 3). No implementation code has been written._
