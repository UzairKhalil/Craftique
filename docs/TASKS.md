...
| [x] T-M0-013  | GitHub Actions CI: MySQL + Redis services, `composer test`, phpstan, pint, eslint, tsc, `npm run build`, `composer audit`, `npm audit`                               | T-M0-005…009       | ✅ `.github/workflows/ci.yml`: two jobs. **quality** (MySQL 8.4 + Redis 7 services) runs doctor, Pint, PHPStan, Rector `--dry-run`, migrations, tests, `composer audit`. **frontend** runs tsc, ESLint, Prettier, client+SSR build, `npm audit --omit=dev`. Every gate verified locally; a planted lint error exits 1 as required. ⚠️ Not yet observed green on GitHub — needs a push                                                                                                              |
